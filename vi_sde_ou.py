...
def KL_ito(z0, alpha, beta, sigma):
    """
        Compute the Riemann approximation to integral in KL divergence
    """
    func_kl = vmap(lambda z: 16*(z**6) - 32* (z**4) + (16-(beta**2) - 12*(sigma ** 2))*(z**2) + 2*alpha*(beta**2)*z)
    
    # select 100 random points between 0 and 1 while including 1 by default
    loc, _ = torch.sort(torch.cat([torch.FloatTensor(100).uniform_(0,1), torch.tensor([1.])]))
    x, delta = simulate_OU_v4(z0, alpha, beta, sigma, loc)
    
    return torch.trapezoid(abs(func_kl(x)), x=loc)


# In[ ]:


def KL_rest(t0, t1, z0, z1, alpha, beta, sigma):
    term1 = 1/(2*(sigma)**2) * (alpha**2 * beta**2 + (4+beta)*sigma**2) * (t1-t0)
